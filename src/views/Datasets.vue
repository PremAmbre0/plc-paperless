  ...
  <div>
    <v-container tag="div" fluid>
      ...
      <v-list subheader two-line>
        ...
        >
          <v-list-item-avatar>
            <v-icon class="grey lighten-1" dark> mdi-file-table </v-icon>
          </v-list-item-avatar>
          ...
          <v-menu>
            ...
            <v-list rounded dense>
              <v-list-item v-for="(option, index) in options" :key="index">
                <v-list-item-title
                  @click="eventHandler(option.value, dataset._id)"
                  >{{ option.value }}
                </v-list-item-title>
              </v-list-item>
            </v-list>
    ...
    <dialog-form :existingFormData="selectedDataset" @reloadData="getData">
    </dialog-form>
...
export default {
  data() {
    return {
      datasetsList: null,
      options: [{ value: "View" }, { value: "Delete" }, { value: "Download" }],
      mode: "",
      selectedDataset: {},
    };
  },
  ...
  methods: {
    ...
    eventHandler(option, id) {
      if (option == "Delete") {
        this.deleteDataset({ _id: id }).then((response) => {
          this.getData();
